...
if(version == "A"){
  ...
  # Version A - Low taxonomic resolution but low data discard
  # Around 5 % of transects with total percentage cover > 101 %
  ...
  read_csv("data/01_raw-data/benthic-cover_paths.csv") %>% 
    ...
    select("SAMPLE.CODE", "YEAR", "REGION", "LOCATION", "SITE.NAME", "DEPTH.ZONE", "TRANSECT",
           where(~ any(.x == "Benthic Category", na.rm = TRUE))) %>% 
    filter(!(row_number() %in% c(1860, 1861))) %>% 
    pivot_longer(8:ncol(.), values_to = "measurementValue", names_to = "organismID") %>% 
    ...
    mutate(locality = str_squish(locality),
           ...
           organismID = str_remove_all(organismID, "..total."),
  ...
  # Version B - High taxonomic resolution but high data discard
  # Around 30 % of transects with total percentage cover > 101 %
  ...
  read_csv("data/01_raw-data/benthic-cover_paths.csv") %>% 
    ...
    filter(!(organismID %in% c("Abiotic..total.", "Anemones..total.", "Macroalgae..total.", "Turf.Algae..total.",
                               ...
                               "Seagrass..total.", "Peyssonneliaceae..total."))) %>% 
    write.csv(., file = paste0("data/02_standardized-data/", dataset, ".csv"), row.names = FALSE)
  
}else{
  
  stop("The name of the version must be either A or B")
