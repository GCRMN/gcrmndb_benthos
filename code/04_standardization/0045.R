...
dataset <- "0045" # Define the dataset_id
...
data <- mermaid_get_project_data(project = project_id, data = "observations", method = "benthicpit")

# 4. Save raw data ----

write.csv(data, file = paste0("data/01_raw-data/", dataset, "/mermaid_",
                              project_id, "_", lubridate::date(Sys.time()), ".csv"),
          row.names = FALSE)
