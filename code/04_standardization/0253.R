...
dataset <- "0253" # Define the dataset_id
...
read_csv("data/01_raw-data/benthic-cover_paths.csv") %>% 
  ...
  mutate(across(c("decimalLatitude", "decimalLongitude", "year", "month", "verbatimDepth"), ~as.numeric(.x)),
         ...
         samplingProtocol = str_replace_all(samplingProtocol,
                                            c("Point_intercept_transect" = "Point intercept transect"))) %>% 
