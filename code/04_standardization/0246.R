...
dataset <- "0246" # Define the dataset_id
...
read_csv("data/01_raw-data/benthic-cover_paths.csv") %>% 
  ...
  mutate(across(c("decimalLatitude", "Hardcoral_percent", "Softcoral_percent",
                  "Reckilledcoral_percent", "Macroalgae_percent", "Turfalgae_percent",
                  "Corallinealgae_percent", "Other_percent", "verbatimDepth"), ~as.numeric(.x))) %>% 
  ...
  mutate(organismID = str_remove_all(organismID, "_percent"),
         samplingProtocol = str_replace_all(samplingProtocol,
                                            c("Line_intercept_transect" = "Line Intercept Transect",
