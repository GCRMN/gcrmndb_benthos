...
synthetic_data <- map(files_list, ~read_csv(., show_col_types = FALSE)) %>% 
  list_rbind()
...
if(length(current_unique_organismID) != 0){
  ...
  current_unique_organismID <- map(1:length(current_unique_organismID),
...
synthetic_data <- st_join(synthetic_data_coords, data_gcrmn_ecoregions) %>% 
...
data_eez <- st_read("../data/07_data-eez/02_clean/eez_v12.shp") %>% 
  # Correct sovereignty issues
  mutate(SOVEREIGN1 = case_when(TERRITORY1 == "Aruba" ~ "Aruba",
                                TERRITORY1 == "Curaçao" ~ "Curaçao",
                                TERRITORY1 == "Sint-Maarten" ~ "Sint-Maarten",
                                TRUE ~ SOVEREIGN1))
...
reef_buffer <- st_read("../data/08_quality-checks-buffer/03_reefs-buffer/reef_buffer_100.shp") %>% 
