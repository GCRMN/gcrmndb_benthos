...
# 1. Misc. corrections on taxid levels ----

synthetic_data <- synthetic_data %>% 
  # Homogenize taxid
  mutate(taxid = str_to_lower(taxid), # To lowercase
         taxid = gsub("[()]", "", taxid), # Remove parentheses
         taxid = iconv(taxid, from = 'UTF-8', to = 'ASCII//TRANSLIT'), # Convert accent letters
         taxid = str_replace_all(taxid, c("&" = "and",
                                          "_" = " ",
                                          "," = " ")), # Convert some special characters
         taxid = str_squish(taxid), 
         taxid = str_trim(taxid)) # Remove duplicated space and space before and after
...
synthetic_data <- st_join(synthetic_data_coords, data_gcrmn_regions) %>% 
  ...
  rename(lat = Y, long = X, region = gcrmn_region) %>% 
  st_drop_geometry() %>% 
...
synthetic_data <- st_join(synthetic_data_coords, data_eez) %>% 
  bind_cols(., st_coordinates(.)) %>% 
  rename(lat = Y, long = X, country = SOVEREIGN1, territory = TERRITORY1) %>% 
  st_drop_geometry() %>% 
  left_join(synthetic_data, .)
...
map(unique(synthetic_data$dataset_id), ~render_rmd(.))
