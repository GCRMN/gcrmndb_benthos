...
synthetic_data <- synthetic_data %>% 
  ...
  summarise(qc8 = round(sum(measurementValue), 5)) %>% 
  ungroup() %>% 
...
quality_checks <- synthetic_data %>% 
  mutate(across(c("qc1", "qc2", "qc3", "qc4", "qc5", "qc6", "qc7", "qc8", "qc9"), 
                ~as.character(.))) %>% 
  mutate(across(c("qc1", "qc2", "qc3", "qc4", "qc5", "qc6", "qc7", "qc8", "qc9"), 
                ~str_replace_all(., 
                                 c("FALSE" = cur_column(), 
                                   "TRUE" = NA_character_)))) %>% 
  mutate(qc = coalesce(qc1, qc2, qc3, qc4, qc5, qc6, qc7, qc8, qc9)) %>% 
  group_by(datasetID, qc) %>% 
  count() %>% 
  ungroup() %>% 
  bind_rows(., tibble(datasetID = NA, qc = c("qc1", "qc2", "qc3", "qc4", "qc5", "qc6", "qc7", "qc8", "qc9"), n = NA)) %>% 
...
synthetic_data <- synthetic_data %>%
  filter_at(c("qc1", "qc2", "qc3", "qc4", "qc5", "qc6", "qc7", "qc8", "qc9"), ~.x == TRUE) %>% 
