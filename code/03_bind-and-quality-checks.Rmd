...
# To avoid deleting sampling unit whose total is between 100 % and 101 %
...
quality_checks <- synthetic_data %>% 
  ...
  mutate(across(c("qc1", "qc2", "qc3", "qc4", "qc5", "qc6", "qc7", "qc8_a", "qc8_b", "qc9"), 
                ~str_replace_all(., 
                                 c("FALSE" = cur_column(), 
                                   "TRUE" = NA_character_)))) %>% 
  mutate(qc = coalesce(qc1, qc2, qc3, qc4, qc5, qc6, qc7, qc8_a, qc8_b, qc9)) %>% 
  group_by(datasetID, qc) %>% 
  count() %>% 
  ungroup() %>% 
  bind_rows(., tibble(datasetID = NA,
                      qc = c("qc1", "qc2", "qc3", "qc4", "qc5", "qc6", "qc7", "qc8_a", "qc8_b", "qc9"),
                      n = NA)) %>% 
